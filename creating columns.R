...
weight_names    <- vector(mode="character", length=ncol(keyword_trimmed))
...
for (add_index in (1:max(data$numkeys)))
{
  if (add_index<=9){
    ...
    weight_names[add_index]    <- paste("k0",add_index ,"_","weight",sep = "")
                   }
    ...
    weight_names[add_index]    <- paste("k",add_index ,"_","weight",sep = "")
...
for(newcol in c(abstract_names,title_names,weight_names))
